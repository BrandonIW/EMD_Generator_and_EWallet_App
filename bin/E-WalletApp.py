...
def _wallet_additional_functions(option, wallet):
    if option == 1:
        ...
        wallet.receive_from_bank(emd_token)
        sleep(1)
...
class Wallet:
    ...
    def receive_sync_token(self, token):
        ...
        receiver_id = str(receiver_id).zfill(4)
        sender_id = str(sender_id).zfill(4)
        ...
        if sender_id not in self.synced_wallets.keys():
            ...
            while True:
                ...
                if choice.lower() in ['y', 'yes']:
                    self.create_sync_token(sender_id)
                    return
        ...
        print(f"Wallet ID-{sender_id} is already synced. Current counter is {self.synced_wallets[sender_id]}\n")
    ...
    def receive_from_bank(self, emd_token_hex):
